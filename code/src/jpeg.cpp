...
//#define JPEG_ENABLE_DEBUG

// Debug functions
#ifdef JPEG_ENABLE_DEBUG
#define JPEG_DEBUG(x) (Serial.println(F(x)))
#else
#define JPEG_DEBUG(x)
#endif
...
{
 static uint8_t b[((WIDTH/2 + 16 - 1) & (~(16 - 1))) *3 * 16];
  void* x = (void*)b;// malloc(nSize);
  if(!x)
  {
    delay(1000);
    Serial.println("jpeg allocation error");
  }
  return x;
}
...
{
  for( ; num_pixels; pDst += 6, pSrc+=4, num_pixels-=2)
  {
#ifdef blackwhite
	  pDst[0] = pSrc[0];
	  pDst[3] = pSrc[2];
	  pDst[1]= pDst[2]= pDst[4]= pDst[5]=128;
#else
...
int encodeimage(output_stream &dst_stream)
{
  ...
  dst_stream.reset_size();
   int width = WIDTH/2;
   int height = HEIGHT;

   JPEG_DEBUG("[jpeg] start encoding image");

   jpeg_encoder dst_image;
    if (!dst_image.init((output_stream*)&dst_stream, width, height, 1,  params()))
   {
    	ERROR_MESSAGE("[jpeg] jpeg error 1\r\n");
      return 0;
   }

    JPEG_DEBUG("[jpeg] encoder initialised");
       for (int i = 0; i < height; i++)
     {

    	    JPEG_DEBUG("[jpeg] encoding line");
    	    STOPWORKING;
        const uint8* pScanline = (const uint8*)getline(i);
    	    STARTWORKING;
          if (!dst_image.process_scanline(pScanline)){
             dst_image.deinit();
             startimageandfreeram();
             dst_stream.init_buffer();
             ERROR_MESSAGE("[jpeg] jpeg error 2\r\n");
             return 0;
         }
    	    STOPWORKING;
         freeline(i);
	    ...
	    JPEG_DEBUG("[jpeg] finish encoding");

     if (!dst_image.process_scanline(NULL)){
         dst_image.deinit();
         dst_stream.init_buffer();
         startimageandfreeram();
         ERROR_MESSAGE("[jpeg] jpeg error 3\r\n");
         return 0;
      }


	    JPEG_DEBUG("[jpeg] finished encoding");
   dst_image.deinit();
   STOPWORKING;
   return (int) dst_stream.get_size();
}
...
int encodeimage(output_stream &dst_stream)
{
   const char* header = "FFFFFFFFFF";
   dst_stream.reset_size();
   dst_stream.put_buf(header,10);
      for (int i = 0; i < HEIGHT; i++)
     {
        JPEG_DEBUG("encoding line");
        const uint8* pScanline = (const uint8*)getline(i);
        for(int e = 0; e < WIDTH; e+=2)
        	dst_stream.put_buf(&pScanline[e],1);
        freeline(i);
     }
   JPEG_DEBUG("finish encoding");
