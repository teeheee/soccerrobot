...
#ifdef XBEE_ENABLE_DEBUG0
// Debug is enabled, XBEE_DEBUG is a simple macro that just inserts the code within it's parameter
#define XBEE_DEBUG0(x) (x)
#else
// Debug is not enabled, the XBEE_DEBUG becomes a NOP macro that essentially discards it's parameter
#define XBEE_DEBUG0(x)
#endif

#ifdef XBEE_ENABLE_DEBUG1
// Debug is enabled, XBEE_DEBUG is a simple macro that just inserts the code within it's parameter
#define XBEE_DEBUG1(x) (x)
#else
// Debug is not enabled, the XBEE_DEBUG becomes a NOP macro that essentially discards it's parameter
#define XBEE_DEBUG1(x)
#endif
...
#endif
		XBEE_DEBUG1(Serial.print(F("OUT 0x")));
		XBEE_DEBUG1(Serial.println(data[i], HEX));
		rxtx(data[i]);
	}
}
...
uint8_t XbeeWifi::read()
{
	...
	XBEE_DEBUG1(Serial.print("IN 0x"));
	XBEE_DEBUG1(Serial.println(data, HEX));
	...
	*len = 0;
	...
	do {
		...
		if (!wait_atn(atn_wait_ms)) {
			if (atn_wait_ms > 0) {
				XBEE_DEBUG0(Serial.println(F("****** Failed in rx_frame waiting for ATN")));
			}
		...
		if (in != 0x7E) {
			XBEE_DEBUG(Serial.print(F("****** Failed in rx_frame, invalid start byte: ")));
		...
		switch(type) {
			...
			case XBEE_API_FRAME_ATCMD_RESP		:
				...
				*frame_type = type;
				...
				spiEnd();
				if (truncated) {
					XBEE_DEBUG0(Serial.println(F("****** RX fail, truncation")));
					return RX_FAIL_TRUNCATED;
				} else if (cs != cs_incoming) {
					XBEE_DEBUG0(Serial.println(F("****** RX fail, checksum")));
			...
			default				:
				...
				// Drop it with debug
				XBEE_DEBUG0(Serial.print(F("**** RX DROP Unsupported frame, type : 0x")));
				XBEE_DEBUG0(Serial.println(type, HEX));
				for (unsigned int i = 0 ; i < rxlen + 1; i++) {
#ifdef XBEE_ENABLE_DEBUG
					uint8_t dropped = read();
				XBEE_DEBUG0(Serial.print(F("Dropping : 0x")));
					XBEE_DEBUG0(Serial.println(dropped, HEX));
#else
					read();
#endif
				}
...
bool XbeeWifi::at_cmd(const char *atxx, const uint8_t *parmval, int parmlen, void *returndata, int *returnlen, bool queued)
{
	...
	if (!queued && (atxx[0] != 'A' || atxx[1] != 'S')) {
		...
		XBEE_DEBUG(Serial.println(F("AT response expected")));
		uint8_t type;
		unsigned int len;
		int res;
...
bool XbeeWifi::at_remcmd(uint8_t *ip, const char *atxx, const uint8_t *parmval, int parmlen, void *returndata, int *returnlen, bool apply)
{
	...
	if ((res = rx_frame(&type, &len, buf, XBEE_BUFSIZE)) == RX_SUCCESS) {
		if (type == XBEE_API_FRAME_REMOTE_CMD_RESP &&
			...
			XBEE_DEBUG0(Serial.println(F("****** Failed AT/REM CMD RESP")));
		}
	} else {
		XBEE_DEBUG0(Serial.print(F("****** Failed AT/REM CMD RESP, error ")));
		XBEE_DEBUG0(Serial.println(res, DEC));
	}
	return false;
}
...
bool XbeeWifi::at_query(const char *atxx, uint8_t *parmval, int *parmlen, int maxlen)
{
	...
	if (at_cmd(atxx, NULL, 0, retbuf, &len, false)) {
		*parmlen = len;
		...
		XBEE_DEBUG0(Serial.println(F("****** Failed AT QRY")));
		return false;
	}
}
...
// parmlen will return the length of the parameter
// if maxlen < parmlen then parmval will be truncated
bool XbeeWifi::at_query_str(const char *atxx,uint8_t* input_param,int input_param_len, uint8_t *parmval, int *parmlen, int maxlen)
{
	//	return at_cmd(atxx, (uint8_t *)buffer, strlen(buffer), NULL, 0, queued);
	char retbuf[XBEE_BUFSIZE];
	int len;

	if (at_cmd(atxx, input_param, input_param_len, retbuf, &len, false)) {
		*parmlen = len;
		memcpy(parmval, retbuf, len > maxlen ? maxlen : len);
		return true;
	} else {
...
bool XbeeWifi::at_remquery(uint8_t *ip, const char *atxx, uint8_t *parmval, int *parmlen, int maxlen)
{
	...
	if (at_remcmd(ip, atxx, NULL, 0, retbuf, &len, true)) {
		*parmlen = len;
		...
		XBEE_DEBUG0(Serial.println(F("**** Failed ATREMQRY")));
		return false;
	}
}
...
void XbeeWifi::rx_ip(unsigned int len, uint8_t frame_type)
{
	...
	cs = 0xFF - cs;
	if (inbound_cs != cs) {
		XBEE_DEBUG0(Serial.println(F("****** CS Fail inbound rx")));
...
int XbeeWifi::transmit(const uint8_t *ip, s_txoptions *addr, uint8_t *data, int len, bool confirm, bool useAppService)
{
	...
	if (confirm) {
		...
		if (rx_frame(&type, &len, buf, XBEE_BUFSIZE, 10000L) == RX_SUCCESS) { ///CHANGED TIMOUT
			if (type != XBEE_API_FRAME_TX_STATUS) {
				// Did not get the expected frame back
				// Probably not the best idea, but clean out the SPI bus
				XBEE_DEBUG0(Serial.println(F("****** Receive of frame not TX status")));
				flush_spi();
				return 0xff;
			}
			if (buf[0] != next_atid) {
				...
				XBEE_DEBUG0(Serial.println(F("****** Receive of frame, ATID mismatch")));
				flush_spi();
				return 0xff;
			}
			if (buf[1] != 0x00) {
				// Transmission operation success, but failed to transmit
				XBEE_DEBUG0(Serial.print(F("****** TX Failure, code=")));
				XBEE_DEBUG0(Serial.println(buf[1], HEX));
				return buf[1];
			}
		} else {
