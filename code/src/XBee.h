...
#define XBEE_AT_INFRASTRUCTURE_MODE "CE"
#define XBEE_AT_ADDR_DEST_ADDR			"DL"
#define XBEE_AT_DNS_LOOKUP				"AL"
...
class XbeeWifi
{
	...
	bool at_query_str(const char *atxx,uint8_t* input_param,int input_param_len, uint8_t *parmval, int *parmlen, int maxlen);
#endif
	...
	int transmit(const uint8_t *ip, s_txoptions *addr, uint8_t *data, int len, bool confirm = true, bool useAppService = false);
...
#ifndef XBEE_OMIT_RX_data
	//virtual
	void dispatch(uint8_t *data, int len, s_rxinfo *info);
#endif
